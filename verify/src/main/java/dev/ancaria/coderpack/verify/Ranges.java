 ...
 * player is offered, and {@code Ranges} in coderpack's zygote, which refuses a
 * jar at load time. A change here belongs in both.
 *
 * <p>Public because the Gradle plugin checks an author's range before it writes
 * it into a descriptor, and this repository's rule is that anything the plugin,
 * the linter and the scaffolder have to agree about lives in {@code verify} and
 * is read from there.
 */
