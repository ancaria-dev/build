...
dependencies {
    ...
    // Kotlin extensions over the API, in dev.ancaria.coderpack.ktx. They
    // forward to the Java API and do nothing else, and the generated entrypoint
    // uses none of them, so this line is optional; drop it and the same mod
    // still compiles. The loader does not hand it over the way it hands over
    // the API, so it is packed like the standard library above rather than
