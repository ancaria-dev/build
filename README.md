...
`dev.ancaria.coderpack:api-kotlin`, тот же API загрузчика, записанный на
Kotlin. Точка входа наследуется от класса `SacredMod` из этого модуля — он
хранит контекст и передаёт его в `Context.load()` как receiver, — а слушатель
регистрируется как `on<Hero> { }`, а не аннотированным методом. Новых
возможностей модуль не даёт: каждое объявление вызывает метод из Java API.
События остаются только для чтения, а слушатель решаемого события отвечает
через `mutate { Gold.Mutation.change(it.value * 2) }` внутри `on<Gold> { }`.
Убрать зависимость и вернуться к `@Subscribe` —
поддерживаемый способ писать мод на Kotlin. Модуль подключается как
`implementation`, а не `compileOnly`, потому что API загрузчик моду выдаёт, а
этот модуль — нет.
...
который не входит контракт `Verifier.API`, сейчас это `"2"`.
